...
import { validateRequest, type RequestValidation } from '@/middleware/requestValidation.js';
...
export const submit = validateRequest(submitRequestSchema, async (req, res) => {
    ...
    return res
    .status(200)
    .send({ submissionId, status, batchErrors: fileErrors, inProcessEntities: entityList });
})