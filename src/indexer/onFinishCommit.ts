...
const findCategoryMapping = (mappings: string | undefined, categoryId: string) => {
    return  mappings?.split(',').map(mapping => mapping.split(":")).find((pair) => pair[0] === categoryId)
}
...
export const onFinishCommitCallback = (resultOnCommit: ResultOnCommit) => {
    ...
    logger.info(`Records to index: inserts:${data?.inserts.length}, updates:${data?.updates.length}, deletes: ${data?.deletes.length}`);
